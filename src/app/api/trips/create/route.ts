...
export async function POST(request: NextRequest) {
  ...
  const dayTitles = Object.fromEntries(tripDays.map((day) => [day.date, day.title]));
  ...
  return NextResponse.json({ tripId: trip.id, token, state: { revision: 1, dayTitles, cards: seedCards, scheduledItems: seedSchedule } });
}
